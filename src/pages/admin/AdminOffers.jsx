...
export default function AdminOffers() {
  ...
  const [form, setForm] = useState({ title: '', description: '', occasion: 'diwali', discountPercent: '', code: '', validTill: '' })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setShowForm(false)
      setForm({ title: '', description: '', occasion: 'diwali', discountPercent: '', code: '', validTill: '' })
      load()
    } catch (err) {
          ...
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}
            ...
            <form onSubmit={handleSubmit}>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                ...
                <div>
                  <label className="text-sm font-medium text-warm-700 mb-1 block">Valid Till</label>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {offers.map(o => (
          <div key={o._id} className={`bg-white rounded-2xl p-5 shadow-sm border ${o.isActive ? 'border-primary-200' : 'border-warm-100 opacity-60'}`}>
            ...
            {o.validTill && <p className="text-xs text-warm-400 mt-2">Valid till {new Date(o.validTill).toLocaleDateString()}</p>}
